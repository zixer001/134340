	...
	<div class="pcoded-content">
		<div class="row">
			<div class="col-md-12">
				<div class="card">
					<div class="card-body">
					    <h5 class="text-dark">ประวัติรายการ KBANK
...
//echo $test;
	...
	if ($act==100){
	$query = "SELECT * FROM reportkbank ORDER BY id DESC limit 100" or die("Error:" . mysqli_error());
	}elseif ($act==500){
	$query = "SELECT * FROM reportkbank ORDER BY id DESC limit 500" or die("Error:" . mysqli_error());
	}elseif ($act==1000){
	$query = "SELECT * FROM reportkbank ORDER BY id DESC limit 1000" or die("Error:" . mysqli_error());
	}elseif ($act=='all'){
	$query = "SELECT * FROM reportkbank ORDER BY id DESC" or die("Error:" . mysqli_error());
	}else{
		$query = "SELECT * FROM reportkbank ORDER BY id DESC" or die("Error:" . mysqli_error());
	}
	...
	echo "<tr align='center'>
			...
			<th align='center'>เวลาที่ลงบันทึก</th>
	...
	echo '<tbody>';
		...
		echo'<tr>';
			...
			<td align='center'>
				<?php 
			if ($row["type"]=="รับโอนเงิน") {
				echo"<button class='btn btn-primary'>รับโอนเงิน</button>";
			}
				elseif ($row["type"]=="โอนเงิน") {
					echo"<button class='btn btn-success' disabled>โอนเงิน</button>";
				}
				else{
					echo"<button class='btn btn-danger' disabled>อื่นๆ</button>";
				}
			?>
				<?php
			...
			if ($row["fromacc"]!='') {
				echo "<td align='center'>" .$row["fromacc"] .  "</td> ";
			}elseif ($row["toacc"]!=''){
				echo "<td align='center'>" .$row["toacc"] .  "</td> ";
			}elseif ($row["toacc"]=='') {

				if ($row["fromacc"]=='') {
					echo "<td align='center'>ไม่ทราบ</td> ";
					
				}
				
			}
			if ($row["frombank"]!='') {
				echo "<td align='center'>" .$row["frombank"] .  "</td> ";
			}elseif ($row["tobank"]!=''){
				echo "<td align='center'>" .$row["tobank"] .  "</td> ";
			}elseif ($row["frombank"]=='') {
			...
			if ($row["fromname"]!='') {
				echo "<td align='center'>" .$row["fromname"] .  "</td> ";
			}elseif ($row["toname"]!=''){
				echo "<td align='center'>" .$row["toname"] .  "</td> ";
			}elseif ($row["fromname"]=='') {
