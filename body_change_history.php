    ...
    <script type="text/javascript" src="datetimepicker/jquery.datetimepicker.js"></script>
		<div class="row">
			<div class="col-md-12">
				<div class="card">
					<div class="card-body">
					    <h5 class="text-dark">ค้นหาประวัติแลกพ้อยด์
...
<?php
	...
	$query = "SELECT * FROM changepoint WHERE username LIKE '%$test%' ORDER BY id DESC" or die("Error:" . mysqli_error());
	}elseif ($act==100){
	$query = "SELECT * FROM changepoint ORDER BY id DESC LIMIT 100" or die("Error:" . mysqli_error());
	}elseif ($act==500){
	$query = "SELECT * FROM changepoint ORDER BY id DESC LIMIT 500" or die("Error:" . mysqli_error());
	}elseif ($act==1000){
	$query = "SELECT * FROM changepoint ORDER BY id DESC LIMIT 1000" or die("Error:" . mysqli_error());
	}elseif ($act==all){
	$query = "SELECT * FROM changepoint ORDER BY id DESC" or die("Error:" . mysqli_error());
	}elseif ($act==400){
	$act = (isset($_GET['act']) ? $_GET['act'] : '');
    $from = $_GET['From'];
    $to = $_GET['To'];
    $to2= date('Y-m-d', strtotime($to . '+1 day'));
	$query = "SELECT * FROM changepoint WHERE date_change BETWEEN '$from' AND '$to2' ORDER BY id DESC" or die("Error:" . mysqli_error());
	}
...
<?php
	...
	echo "<tr align='center'>
			...
			<th align='center'>เครดิตที่แลก</th>
	...
	echo '<tbody>';
		...
		echo'<tr>';
			...
			echo "<td align='center'>" .$row["amount"]*$change_point .  "</td> ";
			echo "<td align='center'>" .$row["date_change"] .  "</td> ";
	...
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form class="form-horizontal ng-pristine ng-valid" method="POST" action="changepoint_dp.php">
					<div class="form-group">
						<div class="row">
