	...
	<div class="pcoded-content">
		<div class="row">
			<div class="col-md-12">
				<div class="card">
					<div class="card-body">
					    <h5 class="text-dark">ประวัติรายการ TRUE WALLET
...
//echo $test;
	...
	if ($act==100){
	$query = "SELECT * FROM reporttrue ORDER BY id DESC limit 100" or die("Error:" . mysqli_error());
	}elseif ($act==500){
	$query = "SELECT * FROM reporttrue ORDER BY id DESC limit 500" or die("Error:" . mysqli_error());
	}elseif ($act==1000){
	$query = "SELECT * FROM reporttrue ORDER BY id DESC limit 1000" or die("Error:" . mysqli_error());
	}elseif ($act==all){
	$query = "SELECT * FROM reporttrue ORDER BY id DESC" or die("Error:" . mysqli_error());
	}
	...
	echo '<tbody>';
		...
		echo'<tr>';
			?>
			<?php echo "<td align='center'>" .$row["id"] .  "</td> "; ?>
			<td align='center'>

				<?php 
			if ($row["status"]=="รับโอนเงิน") {
				echo"<button class='btn btn-primary'>รับโอนเงิน</button>";
			}
				elseif ($row["status"]=="โอนเงิน") {
			...
			echo "<td align='center'>" .$row["trueacc"] .  "</td> ";
			echo "<td align='center'>" .$row["name"] .  "</td> ";
			
			
			echo "<td align='center'>" .$row["datetrue"] .  "</td> ";
			echo'</tr>';
		}
	echo '</tbody>';
	echo "</table>";			
