	...
	<div class="pcoded-content">
		<div class="row">
			<div class="col-md-12">
				<div class="card">
					<div class="card-body">
					    <h5 class="text-dark">ประวัติรายการ SCB
...
//echo $test;
	...
	if ($act==100){
	$query = "SELECT * FROM reportscb ORDER BY id DESC limit 100" or die("Error: $con");
	}elseif ($act==500){
	$query = "SELECT * FROM reportscb ORDER BY id DESC limit 500" or die("Error: $con");
	}elseif ($act==1000){
	$query = "SELECT * FROM reportscb ORDER BY id DESC limit 1000" or die("Error: $con");
	}else{
	$query = "SELECT * FROM reportscb ORDER BY id DESC" or die("Error: $con");
	}
	...
	echo '<tbody>';
		...
		echo'<tr>';
			...
			<td align='center'>
				<?php 
			if ($row["type"]=="ฝากเงิน" OR $row["type"]=="Deposit") {
				echo"<button class='btn btn-primary'>รับโอนเงิน</button>";
			}
				elseif ($row["type"]=="ถอนเงิน" OR $row["type"]=="Withdrawal") {
			...
			echo "<td align='center'>" .$row["date_check"] .  "</td> ";	
			
			
		
			
			echo "<td align='center'>" .$row["time_check"] .  "</td> ";
			echo'</tr>';
		}
	echo '</tbody>';
	echo "</table>";			
